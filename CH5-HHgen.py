...
n0 = 16000 #number of initial walkers
dtau = 10 #time step
m1 = 21894.687
m2 = 1837.515
...
def HHAvgDistDW(whereUfrom, ancestors, histPile): #--------calculating DW for average HH distance in molecule
	HHdist = np.zeros(10)
	xh = np.zeros(5)
	yh = np.zeros(5)
	zh = np.zeros(5)
...
#######                                          #########
#######                                          #########
fileEnergy = open("HHenergies.txt", "a+")
fileXYZ = open("HHancestors.xyz", "a+")
fileHistData = open("HHhistdata.txt", "a+")
fileBinEdges = open("HHbinedges.txt", "a+")
timestep = 0
energies = []
population = []
ancestorPile = []
...
for c in range(0,cycles):    
	...
	if 0 < timestep%500 < 50 and timestep > 4000:
		histPile = HHAvgDistDW(whereUfrom, ancestors, histPile) #--------calculating DW for average HH distance in molecule
